...
func _ready() -> void:
    ...
    capture_requested = user_args.has("capture") or user_args.has("gather_capture") or user_args.has("rain_capture") or user_args.has("kindle_capture")
    agents = [
...
func _advance_fire(delta: float) -> void:
    ...
    if was_lit and fire_heat <= 0.06:
        _record_action("fire_extinguished","",["Regnen efterlader ildstedet som mÃ¸rke, rygende glÃ¸der.","Vandet vinder langsomt over de sidste glÃ¸der."])
